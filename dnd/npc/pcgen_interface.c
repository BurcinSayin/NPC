  ...
  for( feat = feats; feat != 0; feat = feat->next ) {
    if( feat->type == type ) {
      ...
      switch( feat->type ) {
        case ftSPELLMASTERY:
          nfsp = feat->data;
		  fprintf( output, "Spell Mastery(%d&colon;%d&colon;",nfsp->count,nfsp->count);
        ...
        case ftGREATERSPELLFOCUS:
          nfsch = feat->data;
          mixCase( buffer, dndGetSchoolOfMagicName( nfsch->school ) );
		  fprintf( output, "Greater Spell Focus(");
          fprintf( output, "%s", buffer );
		  fprintf( output, ");");
          break;
        ...
        case ftGREATERWEAPONSPECIALIZATION:
		  ...
		  fprintf( output, "%s(",dndGetFeatName( type ));
          fprintf( output, "%s", buffer );
		  fprintf( output, ");");
          break;
        case ftSKILLFOCUS:
  ...
  if( spells != 0 ) {
    ...
    for( i = 0; i < 10; i++ ) {
      for( spell = spells[i]; spell != 0; spell = spell->next ) {
		...
		fprintf( output, "%s(%s %d);",buffer,buffer2,dndGetSpellLevel(cls->type, spell->spell));
      }
    }
  }
...
// Bio Block
	...
	//fprintf( output, "NAME|GENDER|RACE|HEIGHT|WEIGHT|AGE|STR|DEX|CON|INT|WIS|CHA|ALIGN|CLASSES|LEVELS|SKILLS|LANGUAGES|FEATS|DOMAINS|SPELLS\n");
	switch( npc->race ) {
	...
	fprintf( output, "%s|%s|%s|",npc->name,( npc->gender == gMALE ? "Male" : "Female" ),buffer);
	...
	fprintf( output, "%d|",npc->height_ft * 12 + npc->height_in);
	fprintf( output, "%d|",npc->weight);
	//fprintf( output, "%d|",npc->age);
	//fprintf( output, "GENDER:%s\n",( npc->gender == gMALE ? "Male" : "Female" ));

// Attribute Block

	//fprintf( output, "# Character Attributes|");
	fprintf( output, "%d|",npc->strength - dndGetRaceBonus( npc->race, npc->gender, rbtABILITYSCORE, abSTRENGTH ));
	fprintf( output, "%d|",npc->dexterity - dndGetRaceBonus( npc->race, npc->gender, rbtABILITYSCORE, abDEXTERITY ));
	fprintf( output, "%d|",npc->constitution - dndGetRaceBonus( npc->race, npc->gender, rbtABILITYSCORE, abCONSTITUTION ));
	fprintf( output, "%d|",npc->intelligence - dndGetRaceBonus( npc->race, npc->gender, rbtABILITYSCORE, abINTELLIGENCE ));
	fprintf( output, "%d|",npc->wisdom - dndGetRaceBonus( npc->race, npc->gender, rbtABILITYSCORE, abWISDOM ));
	fprintf( output, "%d|",npc->charisma - dndGetRaceBonus( npc->race, npc->gender, rbtABILITYSCORE, abCHARISMA ));

	if (npc->alignment == (alLCNEUTRAL | alGENEUTRAL)) fprintf( output, "TN|");
	else fprintf( output, "%s|",dndGetAlignmentAbbr( npc->alignment ));
...
// Classes Block
	...
	for( cls = npc->classes; cls != 0; cls = cls->next ) {
		...
		fprintf( output, "%d %s:%s%s;",cls->level,buffer2,buffer3,buffer4);
		...
		if (skill->rank > 0) {
			...
			for(i=1;i<100;i++) {
				if (skill->classrank[i] > 0.4f) {
					...
					fprintf( output, "=%g;",skill->classrank[i]);
				}
			}
			//fprintf( output, "|");
		}
...
// Feat Block
  ...
  if (can_turn) {
	if (npc->alignment & alEVIL) fprintf( output, "Rebuke Undead;");
	else fprintf( output, "Turn Undead;");
  }
...
//		case ftRANGERTWOWEAPONFIGHTING:
		  ...
		  continue;
	  case ftEXTRATURNING:
		  fprintf( output, "Extra Turning;");
		  continue;
	  //case ftRANGER
    }
	strcpy( buffer, dndGetFeatName( feat->type ) );
	if (feat->autoAdd) fprintf( output, "%s;", buffer );
  ...
  fprintf( output, "|" );
	  for( cls = npc->classes; cls != 0; cls = cls->next ) {
		if( cls->type == pcCLERIC ) {
		  ...
		  fprintf( output, "DOMAIN:%s;", buffer );
